...
<div class="form-row" id="other-info-section">
	...
	<div class="form-group col-md-4 company-only">
		<?= $this->Form->control('firm_registration_act', [
			'label' => 'Organization Registered Under Act <span class="required">*</span>',
			'escape' => false,
			'class' => 'form-control',
			'required' => true,
			'options' => [
				'The Company Act' => 'The Company Act',
				'The Society Registration Act' => 'The Society Registration Act',
				'Other Act' => 'Other Act'
			],
			'empty' => 'Select Type of Register Act',
		]) ?>
	</div>
	<div class="form-group col-md-4 company-only">
		<?= $this->Form->control('firm_registration_no', [
			'label' => 'Organization Registration No <span class="required">*</span>',
			'escape' => false,
			'class' => 'form-control',
	...
	<div class="form-group col-md-4 individual-only">
 ...
 */
	...
	if (registrationType === 'individual') {
		$('.individual-only').show().find('input, select').prop('required', true);
		$('.company-only').hide().find('input, select').prop('required', false);
	} else {
		$('.company-only').show().find('input, select').prop('required', true);
		$('.individual-only').hide().find('input, select').prop('required', false);
	}
