...
<html lang="en">
	<head>
		...
		<title>BOCW - Labour Cess Portal</title>
	...
	<body class="hold-transition sidebar-mini layout-fixed">
		<div class="wrapper">
			...
			<nav class="main-header navbar navbar-expand navbar-white navbar-light">
				...
				<h3 class="text-center" style="color: red;margin-top: 10px; text-align: center;">Building and Other Construction Workers Welfare Cess</h3>
				<ul class="navbar-nav ml-auto">
					<li class="nav-item dropdown">
						<a class="nav-link" data-toggle="dropdown" href="#">
							<?php
								$session = $this->request->getSession();
								$front = $session->check('front') ? $session->read('front') : null;
								$off_name = $front->email ?? 'Not Set';
								$role     = $front->role ?? 'Not Set';
								?>
							<?php if ($front): ?>
					...
					<li class="nav-item dropdown">
						<a class="nav-link" data-toggle="dropdown" href="javascript:void(0)">
							<i class="far fa-user"></i> &nbsp; <?php echo $session->read('front.username') ?>
						</a>
						<div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
							<a class="dropdown-item" href="<?php echo webURL?>front/changePassword"><i class="fas fa-edit mr-2"></i> Change Password</a>
			...
			<?php echo $this->element('frontsidebar');?>
